...
class SearchServerTest < MiniTest::Unit::TestCase
  ...
  def test_returns_an_index
    search_server = CustomElasticsearch::SearchServer.new("http://l", schema_config, ["mainstream_test", "page-traffic_test"], ["mainstream_test"], SearchConfig.new)
    index = search_server.index("mainstream_test")
    assert index.is_a?(CustomElasticsearch::Index)
    assert_equal "mainstream_test", index.index_name
  end

  def test_raises_an_error_for_unknown_index
    search_server = CustomElasticsearch::SearchServer.new("http://l", schema_config, ["mainstream_test", "page-traffic_test"], ["mainstream_test"], SearchConfig.new)
    assert_raises CustomElasticsearch::NoSuchIndex do
  ...
  def test_can_get_multi_index
    ...
    assert index.is_a?(CustomElasticsearch::IndexForSearch)
